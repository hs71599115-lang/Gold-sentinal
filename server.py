...
ONESIGNAL_WORKER=BASE_DIR/"OneSignalSDKWorker.js"

lock=threading.Lock()
state={
    "service":"Gold Sentinel",
    "mode":"LIVE" if API_KEY else "WAITING_FOR_API_KEY",
    "symbol":SYMBOL,
    "interval":INTERVAL,
...
def fetch_candles():
    if not API_KEY:
        raise RuntimeError("TWELVE_DATA_API_KEY is not set in Render.")
    ...
    vals=p.get("values")
    if not vals:
        raise RuntimeError("No candle data returned. Check your Twelve Data plan supports XAU/USD.")
    return [{
        "t":x.get("datetime"),
        "o":float(x["open"]),
        "h":float(x["high"]),
...
def analyze(c):
    ...
    e20=ema(closes,20); e50=ema(closes,50); rv=rsi(closes); av=atr(c)
    if None in (e20,e50,rv,av) or av<=0:
        return {"decision":"WAIT","signal":None}

    price=closes[-1]
    bull=e20>e50 and 52<=rv<=70
    bear=e20<e50 and 30<=rv<=48
    ...
    if score<MIN_SCORE or not (bull or bear):
        return res
...
def scan():
    while True:
        try:
            ...
            if r.get("signal"):
                print("SIGNAL",json.dumps(r["signal"]),flush=True)
    ...
    def do_GET(self):
        ...
        if path=="/":
            if not INDEX_FILE.exists():
                return self.sendj({"error":"index.html not found"},500)
            return self.send_bytes(INDEX_FILE.read_bytes(),"text/html; charset=utf-8")

        if path=="/status":
            with lock:
                s=dict(state)
            s["healthy"]=s["error"] is None
            s["note"]="Live alert backend; no automatic trade execution."
            return self.sendj(s)

        if path=="/OneSignalSDKWorker.js":
            if not ONESIGNAL_WORKER.exists():
                return self.sendj({"error":"OneSignalSDKWorker.js not found"},404)
            return self.send_bytes(ONESIGNAL_WORKER.read_bytes(),"application/javascript; charset=utf-8")
